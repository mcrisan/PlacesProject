...
//        $this->em->refresh($places); 
    ...
    function setVoteAction() {
        ...
        $ratingBeforeUpdate = $this->em->getRepository('BundleProjectBundle:PlaceRatings')
                ->getPlaceUserRating($placeId);
        if (empty($ratingBeforeUpdate)) { //if empty - insert new rating
            $insertRating = $this->em->getRepository('BundleProjectBundle:PlaceRatings')
                    ->insertRating($placeId, $voteValue, $this->em);
        } else { //if !empty update current rating
            $currentCounts = $this->em->getRepository('BundleProjectBundle:PlaceRatings')
                    ->getCurrentCounts($placeId);
            $currentVotes = $this->em->getRepository('BundleProjectBundle:PlaceRatings')
                    ->getCurrentVotes($placeId);
            $updateRating = $this->em->getRepository('BundleProjectBundle:PlaceRatings')
